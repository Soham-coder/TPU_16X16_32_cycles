...
$library work
$compile -writetoplevels questa.tops -timescale 1ps/1ps -f run_tpu.f 
$sim -f questa.tops -c -do "vsim -voptargs=+acc=npr; run -all; exit" -voptargs=+acc=npr | tee $LOG/log_tpu.txt
